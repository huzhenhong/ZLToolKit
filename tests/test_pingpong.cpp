...
{
  public:
    EchoSession(const Socket::Ptr& pSock)
        : Session(pSock)
    {
        DebugL;
    }
...
class CMD_pingpong : public CMD
{
  public:
    CMD_pingpong()
    {
        ...
        // 指定服务器地址
        (*_parser) << Option('s', "server", Option::ArgRequired, "127.0.0.1:10000", false, "客户端模式：测试服务器地址", [](const std::shared_ptr<ostream>& stream, const string& arg)
                             {
