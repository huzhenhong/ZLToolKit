...
namespace toolkit
{
    ...
    LogContext::LogContext(LogLevel level, const char* file, const char* function, int line, const char* module_name, const char* flag)
        : _level(level), _line(line), _file(getFileName(file)), _function(getFunctionName(function)), _module_name(module_name), _flag(flag)
    {
    ...
    AsyncLogWriter::AsyncLogWriter()
        : _exit_flag(false)
    {
        _thread = std::make_shared<thread>([this]()
                                           { this->run(); });
    }
    ...
    void SysLogChannel::write(const Logger& logger, const LogContextPtr& ctx)
    {
        ...
        syslog(s_syslog_lev[ctx->_level], "## %s %s | %s %s\r\n", printTime(ctx->_tv).data(), LOG_CONST_TABLE[ctx->_level][2], ctx->_function.data(), ctx->str().data());
    }
    ...
    std::string LogChannel::printTime(const timeval& tv)
    {
        ...
        char buf[128];
        snprintf(buf, sizeof(buf), "%d-%02d-%02d %02d:%02d:%02d.%03d", 1900 + tm.tm_year, 1 + tm.tm_mon, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec, (int)(tv.tv_usec / 1000));
        return buf;
    }
